  ...
  return (
    ...
    <div className="absolute bottom-4 left-4 space-y-3">
        ...
        <div className="hud-panel rounded-lg p-4 pointer-events-auto min-w-[200px]">
            <HealthBar current={player.health} max={100} type="health" className="mb-2" />
            <HealthBar current={player.armor} max={100} type="armor" />
        </div>
        {/* Weapon Selector */}
        <div className="hud-panel rounded-lg p-3 pointer-events-auto">
            <WeaponSelector
    ...
    <div className="absolute top-4 right-4 w-32 h-32 minimap pointer-events-auto">
        <div className="relative w-full h-full">
            ...
            {/* Player dot - Handle 3D position */}
            <div
                className="absolute player-dot w-2 h-2"
                style={{
                    left: `${(player.position.x || player.position[0] || 400) / 800 * 100}%`,
                    top: `${(player.position.y || player.position[1] || 300) / 600 * 100}%`,
                    transform: "translate(-50%, -50%)"
                }} />
            {/* Enemy dots - Handle 3D positions */}
            {match?.players?.filter(p => p.alive && p.id !== player.id).map(enemy => <div
                key={enemy.id}
                className="absolute enemy-dot w-1.5 h-1.5"
                style={{
                    left: `${(enemy.position.x || enemy.position[0] || 0) / 800 * 100}%`,
                    top: `${(enemy.position.y || enemy.position[1] || 0) / 600 * 100}%`,
                    transform: "translate(-50%, -50%)"
                }} />)}
        </div>
    </div></motion.div>
